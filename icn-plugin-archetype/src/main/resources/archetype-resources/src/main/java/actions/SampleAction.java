...
import static ${package}.configuration.AdditionalConfigurationBuilder.newConfiguration;
import static ${package}.configuration.PropertyBuilder.newStringProperty;
...
import ${package}.nls.PluginResources;
...
public class SampleAction extends PluginAction {
	...
	public String getName(Locale locale) {
		return PluginResources.get(getId(), "name", locale);
	}
	...
	public JSONObject getAdditionalConfiguration(Locale locale) {
		...
		return newConfiguration()
			.name(PluginResources.get(getId(), "name", locale))
			.description(PluginResources.get(getId(), "description", locale) )
			.property( newStringProperty()
					.id("label")
					.title(PluginResources.get(getId(), "labelTitle", locale) ) )
			.build();
	}
	...
	public String getActionModelClass() {
		return "${pluginId}Dojo.action.SampleAction";
	}
}
